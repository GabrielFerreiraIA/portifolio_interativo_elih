...
export function FlyerCarousel({ plans, filtered, filter, setFilter }: Props) {
  ...
  return (
    <motion.section
      ...
      {...inViewSection}
      variants={fadeUp}
      className="relative bg-white pt-6 pb-12 sm:pt-8 sm:pb-16 scroll-mt-24"
    >
      <div className="mx-auto max-w-6xl px-5">
        <div className="flex items-end justify-between gap-4">
          <div>
            <Overline>{filtered ? "Combinam com sua busca" : "Vitrine de ofertas"}</Overline>
            <h2 className="mt-3 text-2xl sm:text-3xl font-display font-bold tracking-tight text-navy-950 text-balance">
          ...
          {plans.length > 1 && (
            <div className="hidden sm:flex gap-2">
              <button
                ...
                aria-label="Anterior"
                className="grid place-items-center h-11 w-11 rounded-full bg-white border border-neutral-200 text-neutral-600 hover:text-navy-950 hover:bg-neutral-50 shadow-sm transition-colors"
              >
        ...
        <div className="mt-6 flex flex-col gap-3 border-b border-neutral-200/80 pb-4">
          <div className="flex items-center justify-between gap-3 w-full">
            {/* Categorias - Segmented Control / Tabs */}
            <div className="relative flex items-center bg-neutral-100 border border-neutral-200/80 rounded-full p-1 overflow-x-auto no-scrollbar scroll-smooth">
              {[
                { id: null, label: "Todos" },
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      active ? "text-navy-950" : "text-neutral-600 hover:text-navy-950"
                    )}
                  >
                    {tab.label}
                    {active && (
                      <motion.div
                        ...
                        className="absolute inset-0 bg-white rounded-full -z-10 shadow-sm border border-neutral-200/30"
                      />
                    )}
                  </button>
                );
            ...
            <button
              ...
              className={cn(
                "flex items-center gap-2 rounded-full px-3.5 py-2 text-xs sm:text-sm font-semibold transition-all border shrink-0 font-sans shadow-sm",
                showSubFilters || hasSubFilters
                  ? "bg-navy-900 text-white border-transparent"
                  : "bg-white text-neutral-700 border-neutral-200 hover:bg-neutral-50 hover:text-navy-950 hover:border-neutral-300"
              )}
            >
              ...
              {activeSubFiltersCount > 0 && (
                <span className={cn(
                  ...
                  showSubFilters || hasSubFilters ? "bg-white text-navy-900" : "bg-navy-900 text-white"
                )}>
          ...
          <AnimatePresence initial={false}>
            {showSubFilters && (
              ...
              >
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 p-4 mt-2 rounded-panel bg-white border border-neutral-200 shadow-lg font-sans">
                  {/* Para Quem */}
                  <div className="flex flex-col gap-1">
                    <label className="text-[10px] font-semibold text-neutral-500 tracking-wider uppercase">Para quem?</label>
                    <div className="relative">
                      <select
                        value={filter.paraQuem || ""}
                        onChange={(e) => setFilter({ ...filter, paraQuem: (e.target.value as ParaQuem) || null })}
                        className="w-full appearance-none bg-white text-navy-950 border border-neutral-200 hover:border-neutral-300 rounded-lg pl-3 pr-8 py-2 text-xs sm:text-sm font-semibold transition-all focus:outline-none focus:ring-2 focus:ring-sky-100 focus:border-sky-500 cursor-pointer"
                      >
                        <option value="" className="bg-white text-navy-950">Qualquer público</option>
                        {(Object.keys(PARA_QUEM_LABEL) as ParaQuem[]).map((k) => (
                          <option key={k} value={k} className="bg-white text-navy-950">
                            {PARA_QUEM_LABEL[k]}
                          </option>
                        ))}
                      </select>
                  ...
                  {/* Região */}
                  <div className="flex flex-col gap-1">
                    <label className="text-[10px] font-semibold text-neutral-500 tracking-wider uppercase">Região de cobertura</label>
                    <div className="relative">
                      <select
                        value={filter.regiao || ""}
                        ...
                        className="w-full appearance-none bg-white text-navy-950 border border-neutral-200 hover:border-neutral-300 rounded-lg pl-3 pr-8 py-2 text-xs sm:text-sm font-semibold transition-all focus:outline-none focus:ring-2 focus:ring-sky-100 focus:border-sky-500 cursor-pointer"
                      >
                        <option value="" className="bg-white text-navy-950">Qualquer região</option>
                  ...
                  {/* Prioridade */}
                  <div className="flex flex-col gap-1">
                    <label className="text-[10px] font-semibold text-neutral-500 tracking-wider uppercase">Sua Prioridade</label>
                    <div className="relative">
                      <select
                        ...
                        className="w-full appearance-none bg-white text-navy-950 border border-neutral-200 hover:border-neutral-300 rounded-lg pl-3 pr-8 py-2 text-xs sm:text-sm font-semibold transition-all focus:outline-none focus:ring-2 focus:ring-sky-100 focus:border-sky-500 cursor-pointer"
                      >
                        <option value="" className="bg-white text-navy-950">Qualquer prioridade</option>
                  ...
                  {/* Limpar e fechar */}
                  {filtered && (
                    <div className="sm:col-span-3 flex justify-end border-t border-neutral-200 pt-2.5 mt-1">
                      <button
                        type="button"
                        onClick={() => setFilter(emptyFilter)}
                        className="inline-flex items-center gap-1.5 text-xs text-neutral-500 hover:text-navy-950 transition-colors"
                      >
      ...
      >
        {displayPlans.map((plan, index) => (
          ...
          >
            <Link
              to={`/plano/${plan.slug}`}
              className="block relative rounded-panel overflow-hidden border border-neutral-200 shadow-md transition-transform duration-300 hover:scale-[1.02]"
            >
            ...
            <div className="mt-3 flex gap-2">
              <Link
                to={`/plano/${plan.slug}`}
                className="group flex-1 inline-flex items-center justify-center gap-1.5 rounded-full border border-neutral-300 px-3 py-2.5 text-sm font-semibold text-navy-800 hover:bg-neutral-50 hover:border-neutral-400 transition-all font-sans shadow-sm"
              >
                Saiba mais
              ...
              <a
                ...
                className="grid place-items-center h-[44px] w-[44px] rounded-full bg-navy-900 text-white hover:bg-navy-950 shadow-sm hover:shadow-md transition-all shrink-0"
              >
        ...
        {plans.length === 0 && (
          <div className={cn("snap-start shrink-0 w-full rounded-panel bg-neutral-50 border border-neutral-200 p-8 text-center")}>
            <p className="text-neutral-600 font-sans">Nenhuma oferta para esse filtro. Tente ajustar suas escolhas.</p>
          </div>
        )}
      </div>
    </motion.section>
